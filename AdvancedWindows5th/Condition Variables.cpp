...
DWORD WINAPI ProducerThreadProc(PVOID p)
{
	...
	while (true)
	{
		...
		{
			printf("Producer Begin sleep %d\r\n", ProducerId);
			// Buffer is full - sleep so consumers can get items.
			SleepConditionVariableCS(&BufferNotFull, &BufferLock, INFINITE);
			printf("Producer Awake %d\r\n", ProducerId);
		}
...
DWORD WINAPI ConsumerThreadProc(PVOID p)
{
	...
	while (true)
	{
		...
		{
			printf("Consumer Begin sleep %d\r\n", ConsumerId);
			// Buffer is empty - sleep so producers can create items.
			SleepConditionVariableCS(&BufferNotEmpty, &BufferLock, INFINITE);
			printf("Consumer Awake %d\r\n", ConsumerId);
		}
		...
		printf("Consumer wake condition variable %d\r\n", ConsumerId);
...
int main(void)
{
	...
	HANDLE hConsumer3 = CreateThread(NULL, 0, ConsumerThreadProc, (PVOID)3, 0, &id);
